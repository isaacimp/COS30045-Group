...
d3.csv("healthexpenditureaustralia.csv").then(expenditureData => {
    d3.csv("pharmamarketclean.csv").then(dosesData => {
        ...
        const categories = Array.from(new Set(expenditureData.map(d => d.area_of_expenditure)));
        const years = Array.from(new Set(expenditureData.map(d => d.financial_year)));

        const stack = d3.stack()
            .keys(categories)
            .value((d, key) => d.values.find(v => v.area_of_expenditure === key)?.real_expenditure_millions || 0);

        const expenditureByYear = Array.from(d3.group(expenditureData, d => d.financial_year), ([key, values]) => ({ year: key, values }));
        ...
        svgBar.selectAll(".layer")
            ...
            .on("mouseover", function (event, d) {
                tooltip.style("opacity", 1).text(`Year: ${d.data.year}, Value: ${d[1] - d[0]}`);
            })
        ...
        svgLine.selectAll(".dot")
            ...
            .attr("fill", "red")
            .on("mouseover", function (event, d) {
                tooltip.style("opacity", 1).text(`Year: ${d.TIME_PERIOD}, Value: ${d.OBS_VALUE}`);
            })
            .on("mousemove", function (event) {
                tooltip.style("left", `${event.pageX + 10}px`).style("top", `${event.pageY}px`);
            })
